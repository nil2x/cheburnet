...
**cheburnet не может быть использован для полноценной замены VPN и прокси.** API ВКонтакте имеет ограничения, поэтому передать много данных не получится. [Telegram](#telegram) работать будет.
...
Скачайте [stub.jpg](/assets/stub.jpg). Нажмите Фото, нажмите Добавить фото, выберите stub.jpg. Нажмите на загруженное фото. Адресная строка будет иметь вид `https://vk.com/album-12345_6789?z=photo-12345_9876`, где `6789` - ID альбома, а `9876` - ID фото. Запишите оба ID. Вернитесь обратно на страницу сообщества.

Скачайте [stub.mp4](/assets/stub.mp4). Нажмите Видео, наведите на Добавить, нажмите Добавить видео, нажмите Выбрать файл, выберите stub.mp4. Ссылка на видео, справа, будет иметь вид `https://vk.com/video-12345_7890`, где `7890` - ID видео. Запишите этот ID. Нажмите Далее, нажмите Далее, нажмите Опубликовать. Закройте открывшуюся страницу ВК Видео. Вернитесь обратно на страницу сообщества.
...
После удаления cheburnet вы можете отозвать выданный ключ доступа. Откройте ВКонтакте, нажмите на свою аватарку в правом верхнем углу, нажмите Управление аккаунтом VK ID, нажмите Сервисы и сайты, нажмите на сервис которому вы дали доступ, нажмите Отозвать доступ.
...
Откройте [Releases](https://github.com/nil2x/cheburnet/releases), раскройте Assets последней версии, скопируйте ссылку на Linux-версию. Выполните:

```bash
wget -O cheburnet.tar.gz <ССЫЛКА>
mkdir cheburnet
tar -xzf cheburnet.tar.gz -C cheburnet
cd cheburnet
```
...
```bash
mkdir /var/log/cheburnet
touch /var/log/cheburnet/output.log
chown nobody:nogroup /var/log/cheburnet/output.log
mv logrotate.txt /etc/logrotate.d/cheburnet

mkdir /usr/local/etc/cheburnet
mv cheburnet /usr/local/bin/
mv config.json /usr/local/etc/cheburnet/
mv systemd.txt /etc/systemd/system/cheburnet.service

systemctl daemon-reload
systemctl enable cheburnet
```
...
```bash
nano /usr/local/etc/cheburnet/config.json
```

Заполните конфиг значениями которые вы записывали ранее. [Пример](#пример).

- в `log.output` укажите `"/var/log/cheburnet/output.log"`
...
```bash
systemctl status cheburnet
```

Эти команды не должны содержать ошибок:

```bash
journalctl -u cheburnet
```

```bash
tail -n 100 /var/log/cheburnet/output.log
```
...
Откройте [Releases](https://github.com/nil2x/cheburnet/releases), раскройте Assets последней версии, скачайте Windows-версию.
...
Запустите программу `cheburnet.exe` в текущей папке. После закрытия терминала программа будет остановлена.
...
Откройте [Releases](https://github.com/nil2x/cheburnet/releases), раскройте Assets последней версии, скачайте Source code. zip для Windows, tar.gz для Linux и macOS.
...
```bash
docker compose run --rm cheburnet -secret
```
...
Откройте [Releases](https://github.com/nil2x/cheburnet/releases), раскройте Assets последней версии, скопируйте ссылку на Android-версию. Выполните:

```bash
curl -L -o cheburnet.tar.gz <ССЫЛКА>
tar -xzf cheburnet.tar.gz
```
...
Рекомендуется использовать cheburnet в связке с любым [V2Ray-клиентом](#v2ray) для настройки точечного роутинга. Например, отправляйте весь трафик Google через cheburnet, а остальной трафик пускайте напрямую.

Обратите внимание на [Flood control](#flood-control). Не ожидайте быстрой загрузки и стабильного соединения, делайте паузы между запросами, передавайте как можно меньше трафика. Ещё раз: cheburnet расчитан лишь на минимальный доступ к глобальному интернету.
...
```bash
cheburnet -config /path/to/config.json
```
...
```jsonc
{
    ...
    "socks": {
        ...
        // 0 выключает запуск.
        // Рекомендуется установить 0 на устройстве где прокси не используется.
        "port": 1080
    },
    ...
    "imap": [
        {
            ...
            // Адрес сервера.
            // Yandex - imap.yandex.ru, Mail.ru - imap.mail.ru, Rambler - imap.rambler.ru
            "host": "",
            ...
            // Логин пользователя, включая домен (@) почты.
            "username": "",

            // Пароль пользователя.
            // Для Yandex и Mail.ru - отдельный пароль приложения, для Rambler - пароль аккаунта.
            "password": "",

            // Папка где хранятся черновики.
            // Для Yandex и Mail.ru - Черновики, для Rambler - DraftBox.
            "mailbox": ""
        }
    ],
    ...
    "yadisk": [
        {
            // Имя аккаунта для идентификации в логах.
            "name": "",

            // OAuth-токен для приложения.
            "token": ""
        }
    ],
    ...
    "ok": [
        {
            // Имя приложения для идентификации в логах.
            "name": "",
...
{
    "log": {
        "output": "/var/log/cheburnet/output.log"
    },
    "dns": {
        "provider": ""
    },
...
В этом случае `user.accessToken` использоваться не будет, но [Flood control](#flood-control) наступит значительно быстрее.

## Flood control

cheburnet может начать работать нестабильно или вовсе перестать работать. В логах программы вы увидите ошибку `Flood control`. Это значит, что вы передаете слишком много трафика или слишком быстро. VK API имеет ограничения, поэтому передать много данных не получится. Используйте прокси только для минимального доступа к важным сервисам.

Подождите некоторое время для обнуления достигнутых лимитов и повторите запрос. Рекомендуется на время ожидания выключить программу и позже запустить ее повторно. Во время использования сократите количество передаваемого трафика и делайте паузы между запросами.
...
- установите прокси на уровне браузера или даже сайта, а не на уровне всей системы
...
Рекомендуется использовать cheburnet в связке с любым V2Ray-клиентом. В этом случае вы сможете настроить точечный роутинг и обеспечить более широкую поддержку входящих интерфейсов.
...
```text
socks://Og%3D%3D@127.0.0.1:1080#cheburnet
```
...
Скопируйте, откройте клиент, нажмите Настройки, нажмите Настройки маршрутизации, нажмите Добавить, в Примечания напишите cheburnet, нажмите Импорт правил из буфера обмена, нажмите Да, нажмите Подтвердить, нажмите Подтвердить. Внизу, около Маршрутизация, выберите cheburnet.
